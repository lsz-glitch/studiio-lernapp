...
export default function FlashcardAddManualModal({ user, subject, currentCardCount = 0, onClose, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const row = {
        ...
        position: currentCardCount,
      }
      const { error: insertErr } = await supabase.from('flashcards').insert(row)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={onClose}>
      ...
      >
        <h3 className="text-lg font-semibold text-studiio-ink mb-1">Karte manuell hinzufügen</h3>
